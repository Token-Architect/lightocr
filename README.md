...
[![点击播放演示视频](./app/static/demo-cover.jpg)](https://raw.githubusercontent.com/Token-Architect/lightocr/master/app/static/demo.mp4)

> 点击上方封面图即可直接打开 `demo.mp4`（Raw 直链）
...
- `OCR_CPU_LIMIT_PERCENT`：限制进程可用 CPU 比例（如 `80`）
- `OCR_MAX_CONCURRENT_TASKS`：限制同时 OCR 任务数（默认 `2`）
- `OCR_MAX_THREADS`：限制 OpenCV/数值库线程数（如 `2`，`0`表示使用库默认）
- `OCR_MAX_WORKERS`：`vlm_opencv` 模式下每个 PDF 的页并发（如 `3`）
